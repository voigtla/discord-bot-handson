...
👉 無料枠対象で、軽量な用途（Bot・学習用）に向いています。

---

**マシンタイプ**
- **プリセット：e2-micro** を選びます

👉 画面に「無料枠対象」と表示されているものです。  
👉 CPU やメモリを自分で細かく設定する必要はありません。

---

**プロビジョニングモデル**
- **標準** を選びます（デフォルトのままでOK）

👉 割引モデル（Spot など）は、  
　突然停止する可能性があるため **今回の用途には向きません**。

---

#### ブートディスク（OSの設定）

1. **ブートディスク** の「変更」をクリック
2. 次の内容を選びます
...
👉 Linux の種類を指定しているだけです。  
👉 Ubuntu 22.04 は長期サポート版で、資料どおりに進められます。

---

#### ファイアウォール

- **HTTP トラフィックを許可**：チェックしない  
- **HTTPS トラフィックを許可**：チェックしない  

👉 今回は Web サーバーではなく、  
　Discord Bot を裏側で動かすだけなので不要です。

---

#### 作成

すべて確認できたら、画面下の **作成** をクリックします。

数十秒後、VM インスタンス一覧に  
`discord-bot-instance` が表示され、  
**ステータスが「実行中」**になっていれば成功です。
...
第9回では、コードの変更はありません。  
第8回で完成したindex.jsを、サーバー環境にデプロイする方法を学びます。

**使用するファイル：**
- index.js（第8回と同じ）
- ai-helper.js（第6回と同じ）
- spam-detector.js（第7回と同じ）
- content-filter.js（第7回と同じ）
- register-commands.js（第7回までの全コマンドを含む）
- deploy.sh（本章で新規作成）
