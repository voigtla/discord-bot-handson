...
---
了解です。
**第5回 README の「1-2」だけを差し替えられる md 断片**として書き直します。
（＝**部分切り出し**／**mdのみ**／**コードは一切変更しない**）

そのまま **README の 1-2 セクションを丸ごと置き換え**て使えます。
...
このステップでは、  
**Bot起動時に一度だけ実行される「初期データ投入処理」**を定義します。

⚠️ この処理は **「何を書くか」よりも「どこに書くか」** が重要です。

---

#### このステップでやること（概要）

- テンプレート用の初期データを DB に登録する処理を用意する
- **Bot起動時に1回だけ実行**されるようにする
- コマンド処理（`client.on(...)`）の中では **実行しない**

---

#### 貼り付け位置（最重要）

`index.js` の中で、**次の条件をすべて満たす場所**に配置してください。

**✅ 正しい配置条件**

- `Database` に接続した **あと**
- `CREATE TABLE IF NOT EXISTS ...` が **すべて終わったあと**
- `const client = new Client(...)` より **前**
- `client.on('interactionCreate', ...)` の **外**
...
> ❌ `client.on(...)` の中に書かないでください
> ❌ コマンド処理の if 文の中に書かないでください
...
> 「貼らないと先に進めない」ステップではありません。

---

#### 動作確認について

このステップ単体での見た目の変化はありません。

* Discord上で新しいコマンドが増えるわけではありません
* 失敗しても、**すぐに気づきにくい**のが特徴です

そのため、

* 「ここはそういう初期化処理なんだな」と理解できていれば OK
* 次の章に進んでから問題が出た場合に、ここを疑って戻ってきてください

---

✅ **ここまでできたら**
「Bot起動時に1回だけ実行される処理の置き場所」が
イメージできていれば成功です。
